...
- **Decision**: Ship the **adaptive icon** (`mipmap-anydpi-v26/ic_launcher.xml`) with the two layers actually exported — `<foreground>` (`ic_launcher_foreground.png`) and `<background>` (`ic_launcher_background.png`, per-density) — plus the exported **legacy PNG launcher icon** (`ic_launcher.png`) in `mipmap-mdpi…xxxhdpi` for the API 24–25 fallback. Add `android:icon="@mipmap/ic_launcher"` to the manifest `<application>`. **No `android:roundIcon`** — revised decision (2026-07-30, Phase 5 review): the export has no round PNGs, so a round resource could only live under `anydpi-v26` and would be unresolvable at `minSdk = 24`; API 26+ launchers mask the adaptive icon themselves, so it adds nothing. **No monochrome/themed-icon layer** — revised decision (2026-07-13): the design export doesn't include one, so Android 13+ themed home screens fall back to the platform default rather than a tinted La Bella mark (documented edge case, not a defect).
